...
| D4 | Admin passwords | **keep**: both are known and recreated as they are (2B.6); rotate as a follow-up |
...
| 2B.6 | **Users (recorded 2026-09-23): exactly 2, both passwords known, so no hash copying.** `suorantacoding@gmail.com` (the developer; password = `CMS_ACCOUNT_PASSWORD` in `.env.local`) and `simelius.heidi@gmail.com` (Heidi; password = `HEIDI_CMS_PASSWORD` in the owner's file). For each: look up by email first, then `POST $NEW_URL/auth/v1/admin/users` `{email, password, email_confirm:true, app_metadata:{cms_admin:true}}`, built in Node from the env files and never echoed. `password_min_length` (2B.1) must be ≤ the shorter of the two passwords (the script computes it without printing). Check: `POST /auth/v1/token?grant_type=password` for both → 200, and the JWT has `app_metadata.cms_admin=true`. **Never log out with these accounts in tests** (`signOut` is global and would end Heidi's real sessions). The **test admin** `simeliusweb+e2e-<runId>@gmail.com` gets a random 24-char password in `.env.generated`. Fallback only if a password is missing: import the bcrypt hash from Lovable (`select email, encrypted_password from auth.users` → Export CSV → `$STATE`, never shown) with `update auth.users set encrypted_password=…`. | both sign-ins 200 | — |
