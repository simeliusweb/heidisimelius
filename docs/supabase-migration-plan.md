...
| D4 | Admin passwords | **keep**, by importing the bcrypt hashes (2B.6); rotate as a follow-up |
...
| 2B.6 | **Users (hash import, so both keep their current passwords; recorded 2026-09-23):** Lovable has exactly 2 users: `suorantacoding@gmail.com` (the developer, = `CMS_ACCOUNT` in `.env.local`) and `simelius.heidi@gmail.com` (Heidi; password unknown to us). (1) In the Lovable SQL editor (AGENT-BR) run `select email, encrypted_password from auth.users order by email;` and use the result grid's **Export CSV** button. The file lands in `~/Downloads`; move it at once to `$STATE/artifacts/2B/auth-hashes.csv` (chmod 600). **Never read the hashes into chat, logs or screenshots.** (2) For each email: `POST $NEW_URL/auth/v1/admin/users` `{email, password:<random 32>, email_confirm:true, app_metadata:{cms_admin:true}}`, looking up by email first. (3) Via `sbq`: `update auth.users set encrypted_password = '<bcrypt hash>' where email = '<email>'`. The SQL is built from the CSV in a Node script, never echoed. (4) Check: sign in as `suorantacoding@gmail.com` with `CMS_ACCOUNT_PASSWORD` → 200 (proves the hash import works). Heidi's login is proven at 14.4. (5) Delete the CSV. The **test admin** `simeliusweb+e2e-<runId>@gmail.com` gets a random 24-char password in `.env.generated`. Fallback if the import fails: recreate Heidi with a random password; the owner passes it to her at 14.4. | created, and the check passes | — |
